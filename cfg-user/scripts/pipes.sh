...
f=60    # framerate
s=20    # straight probability
r=20000 # reset after ${r} chars
...
case $( uname ) in
  (FreeBSD)
    ...
    if [ -e /usr/local/bin/tput ]
    then
      TPUT=/usr/local/bin/tput
    else
      echo "FreeBSD builtin /usr/bin/tput is too limited."
      echo "Please install the 'devel/ncurses' package."
      exit 1
    fi
  ;;
esac
